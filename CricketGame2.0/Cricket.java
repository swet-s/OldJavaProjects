...
class Cricket
{
static int inn=0,sum1=0,sum0=0;static float rrtt=0;
public static void match(int o,double sec,String t1,String t2)
{
String ob="";long seco=(new Double(sec)).longValue();
Random r=new Random();
Scanner Input=new Scanner(System.in);
float X=r.nextFloat();float toss=Math.round(0+9*X);int ctr=0,rctr=0,wctr=0;
if (toss==0||toss==3||toss==6)
{System.out.println("\n\n "+t1+" won the toss ,and choose to ball first");inn=2;}
else if(toss==8||toss==9)
{System.out.println("\n\n "+t1+" won the toss ,and choose to bat first");inn=1;}
else if(toss==5||toss==2||toss==7)
{System.out.println("\n\n "+t2+" won the toss ,and choose to ball fisrt");inn=1;}
else if(toss==1||toss==4)
{System.out.println("\n\n "+t2+" won the toss ,and choose to bat first");inn=2;}
//game starts now
...
{try{Thread.sleep(1000*seco);}
 catch(InterruptedException ex){Thread.currentThread().interrupt();}
        
rrctr++;int sc=0;octr=bctr/6;System.out.print("\t"+octr+"."+bctr%6+"---");String  os=String.valueOf(octr),bs=String.valueOf(bctr%6);ob=os+"."+bs;
...
if (t==0)  
rtt=rt;System.out.println(":-"+rctr+"-"+wctr+"("+ob+" overs)          Run rate:-"+rt);
if (t==0)
sum0=rctr;
else
sum1=rctr;
